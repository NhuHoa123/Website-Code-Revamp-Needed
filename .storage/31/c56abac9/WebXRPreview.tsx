...
import { OrbitControls, Environment, PresentationControls, Float, Text3D, MeshReflectorMaterial } from '@react-three/drei';
...
const WebXRPreview: React.FC<WebXRPreviewProps> = ({ productId, productName, productType }) => {
  ...
  const enterXR = async (mode: 'ar' | 'vr') => {
    if ('xr' in navigator) {
      try {
        const session = await (navigator as any).xr.requestSession(mode === 'ar' ? 'immersive-ar' : 'immersive-vr');
        setIsXRActive(true);
        setViewMode(mode);
      } catch (error) {
