...
const WebXRPreview: React.FC<WebXRPreviewProps> = ({ productId, productName, productType }) => {
  ...
  const enterXR = async (mode: 'ar' | 'vr') => {
    if ('xr' in navigator) {
      try {
        const xr = (navigator as Navigator & { xr: { requestSession: (mode: string) => Promise<XRSession> } }).xr;
        const session = await xr.requestSession(mode === 'ar' ? 'immersive-ar' : 'immersive-vr');
        setIsXRActive(true);
        setViewMode(mode);
      } catch (error) {
